...
**Path:** `/supplements/verification-plan`
...
Phase 3: Improving
2031–2036+
Verification Plan Timeline - ai-2040.com
...
99.9%
99.99%
Rogue deployment size (H100e, log scale)
P(detected)
within 1 hour
within 1 week
within 1 month
...
Let the verifier's pool be some collection of packets; it could be a single training run, every packet from every workload running under the deal in a given month, or one cluster's traffic. Let 
𝑁
packets
N
packets
 ...
 be the total number of packets in that pool, 
𝑠
=
1
/
...
packets
 ...
 the packet size (the fraction of the pool each packet represents), 
𝐶
C the recomputation budget (the fraction of the pool the verifier can afford to recompute), and 
𝑁
verified
=
𝐶
...
=C/s the number of packets actually audited.

A.2 Detection by Random Sampling

The detection math in this section was independently derived but matches the analysis in Rinberg et al. (2025). Suppose an attacker substitutes 
𝑁
fake
N
fake
 ...
 packets, replacing the declared computation in each with their own unauthorized one. Let
...
be the fraction of packets that are fake. If the verifier audits each packet independently with probability 
𝐶
C, the exact probability that none of the 
𝑁
fake
N
fake
...
, this is well approximated by a Poisson distribution on the number of caught fakes, with mean 
𝜆
=
𝑁
verified
...
so the probability of catching the attack (the detection confidence at attack size 
𝐹
fake
F
fake
...
We'll use the Poisson form throughout, since it's the cleanest expression of the scaling. It breaks down only when 
𝐶
C approaches 1 or 
𝑁
fake
...
fake
 ...
 is single-digit, which is also when random sampling stops buying you anything (see the discussion at the end of §A.3).

A.3 The Assurance Curve

The assurance curve is determined by coverage and confidence which can be expressed in terms of 
𝐹
fake
∗
F
...
, the largest fraction of fake packets we're willing to tolerate without detection:
...
, the fraction of the workload we're claiming is legitimate.
...
, the probability we'd catch any actual violation at or above that threshold.

The confidence formula comes from §A.2: 
𝑃
(
detected
)
...
. The recomputation budget shows up through 
𝑁
verified
=
𝐶
...
, so more audited packets push the curve outward.
...
These curves show the confidence-coverage relationship at different values of 
𝑁
verified
N
verified
...
verified
 ...
 gives you higher assurance (shifts the curve right). Since 
𝑁
verified
=
𝐶
...
=C/s, the curve can be shifted higher either by (1) increasing the recomputation budget 
𝐶
C or (2) shrinking the packet size 
𝑠
s.
...
=10
14
 tokens (100T) using distributed data parallelism. Network taps between server racks log every gradient step (its input batch, the gradient update, and the resulting checkpoint update), so each gradient step is one packet. An attacker wants to insert a backdoor by silently training on 
𝑁
fake tokens
=
10
...
For 99.9% detection confidence we need 
𝑁
verified
⋅
𝐹
...
≥7×10
9
. The recomputation cost as a fraction of the training run is then
...
packets
 ...
 cancels and the detection probability becomes:
...
In this form the detection probability is a function of the recomputation budget 
𝐶
C and the absolute number of fake packets 
𝑁
fake
N
fake
...
≥ln(100)≈4.6, so the attacker is caught with 99% confidence the moment they have accumulated
...
1
%
C=1% that is 461 packets, independent of pool size and of how the work is spread across workloads. Multiplying by the packet size 
𝑝
p converts this to a bound on cumulative rogue compute work:
...
Consider the operational setup of the 2034 section, in which the verifier collects one packet per hour from each frontier GPU (about 100 H100e each by 2034). Then 
𝑝
=
100
p=100 H100e-hours, and with 
𝐶
=
1
%
...
The verifier catches with 99% confidence any rogue internal deployment that accumulates more than about 46K H100e-hours of unapproved compute work inside the verified pool, regardless of pace or distribution. The bound on cumulative rogue work scales linearly in 
𝑝
/
𝐶
p/C; a few representative operating points:

Packet size 
𝑝
p	Budget 
𝐶
C	Max undetected rogue work
...
- If this is reliably bigger than any potential covert project, then it removes the incentive to pull out of the deal for any actor that may have the largest covert project, also reducing the incentive to attempt one in the first place. In our scenario, the US maintains a 5M H100e bank and China a 500K H100e bank (see our deal decline supplement, the source of truth for these stockpiles); the US bank sits comfortably above the ~4M H100e top of the 80% CI for plausible diversion in our covert project supplement. The ratio between the size of the US and China’s compute banks (and whichever third parties might negotiate their own) could be frozen at the pre-deal status quo compute ratio.
...
- Over time we think it may be plausible to rule out covert projects above smaller and smaller sizes, using intelligence efforts, agreements to bring more known compute (e.g., military-exempt compute or tiny private clusters) under privacy-preserving verification measures, or agreements to do power usage accounting (e.g., mutual declaration and random inspection). Our covert project supplement covers the detection methods (waste heat tracking, other IMINT/MASINT, SIGINT, HUMINT) and estimates detection probabilities by covert datacenter size, along with the stronger verification concessions that could be negotiated over time.
- Make sure no new compute goes to covert projects.
- One extremely important property of the deal is that none of the post-deal compute production can go to covert projects. Our covert project supplement estimates that because production runs through a small number of highly controlled fabs, diversion of new chips can be kept to approximately zero, with very high confidence that the total is under 100K H100e. To make this even more robust, it might be favourable to concentrate AI chip production in fewer locations and/or colocate production with the final destinations (datacenters), which is doable because with total production increasing 3x/yr, every 2 years 90% of the fabs are new fabs. This means they can pretty easily be somewhere else. Consumer compute could be produced elsewhere but the risk of covert project usage of consumer compute should be separately mitigated.
...
- Tier 0: Unrestricted
- consumer
- Tier 1: Subject to deal
- inference-only or cold storage
- iPhone 16 Pro Max
- Tesla HW3
- DGX Spark
- M4 Max MacBook
...
- 0.001
- 0.1
- Cross-Chip Interconnect Speed (GB/s)
- Compute (H100e)
- Memory capacity
- tier 1 above 10 TB (scale-up)
- 8 GB
- 128 GB
- 13 TB
- Memory bandwidth
- tier 1 above HBM2 (~1.5 TB/s)
- ≥ ~1.5 TB/s (HBM-class)
- below ~1.5 TB/s
...
- How little regulatory competence is required
- High
- Low
- Regulatory Accuracy
- How well the approach tracks the correct research speed
- Suggested progression
- ~2035
- Safety case burden of proof
- All R&D requires strong safety case
- arguments before approval.
- Quality ad-hoc rules
- Case-by-case decisions on permitted
- research directions and speed.
- Human-interpretable requirement
- All approved techniques need to be
- understood end-to-end by some group of
- humans.
- ~2030
- Compute caps
- Simple caps on R&D compute.
...
- AI-relevant floor: 4,000 TPP ≈ 0.25 H100e (compute supplement §1.1)
- Consumer compute
- below the AI-relevant floor — capped at 30M H100e
- iPhone 16
- Pro Max
- M4 Pro MacBook
- Compute per device (H100e)
- Unverified edge-compute cap
- 30M H100e
- 25M
- 5M
- 25M already in world (pre-deal)
- + 5M new credits
- Verified edge compute → exempt
- Chip Flow Restrictions, 2032 - ai-2040.com
- The product
- (recomputation budget times number of fake packets) is the single quantity that determines 99% catch probability: it just needs to exceed about 4.6.
- This makes the limiting case revealing. Imagine you sample one packet per server-hour, so
- p=1 server-hour, and the attack is also just a single fake packet (one rogue server-hour buried in a much longer, say year-long, workload). With
- =1, the Poisson approximation would say you need
- C≥4.6, which doesn't make sense:
- C is a fraction, capped at 1 (you can't recompute more than 100% of packets).
...
- For
- =1 this gives
- P(catch)=C. To catch a single fake packet with 99% confidence, you'd need to recompute 99% of all packets, i.e. you've essentially given up on sampling and are doing full verification. The size of the surrounding workload doesn't matter: you're flipping a single coin biased by
- C.
- The Poisson formula is accurate whenever
- is at least a few dozen and
- C is small (say under 10%). For
- =100, the exact required budget for 99% catch is 4.5%, vs Poisson's 4.6%, which is very close. For
- =1000 the exact answer is 0.459%, vs Poisson's 0.46%. The two agree better as
- grows. Things only diverge in the small-
- corner, which is also exactly the corner where random sampling stops being useful and you have to recompute everything.
- So the practical takeaway: random sampling buys you a lot once you're catching attacks that span at least dozens of packets. To catch attacks below that, shrink packets (decrease
- p) so the same fake compute-work spans more packets.
...
- [https://ai-2040.com/supplements/verification-plan#summary-of-the-plan](https://ai-2040.com/supplements/verification-plan#summary-of-the-plan)
- [Summary of the Plan](https://ai-2040.com/supplements/verification-plan#summary-of-the-plan)
- [https://ai-2040.com/supplements/verification-plan#phase-1-prepare-deal-optionality](https://ai-2040.com/supplements/verification-plan#phase-1-prepare-deal-optionality)
- [Phase 1. Prepare Deal Optionality](https://ai-2040.com/supplements/verification-plan#phase-1-prepare-deal-optionality)
- [https://ai-2040.com/supplements/verification-plan#phase-2-implement-the-deal](https://ai-2040.com/supplements/verification-plan#phase-2-implement-the-deal)
- [Phase 2. Implement the deal](https://ai-2040.com/supplements/verification-plan#phase-2-implement-the-deal)
- [https://ai-2040.com/supplements/verification-plan#phase-3-improve-robustness](https://ai-2040.com/supplements/verification-plan#phase-3-improve-robustness)
- [Phase 3. Improve Robustness](https://ai-2040.com/supplements/verification-plan#phase-3-improve-robustness)
- [https://ai-2040.com/supplements/verification-plan#verifying-the-deal-in-our-plan-a-scenario](https://ai-2040.com/supplements/verification-plan#verifying-the-deal-in-our-plan-a-scenario)
- [Verifying the deal in our Plan A scenario](https://ai-2040.com/supplements/verification-plan#verifying-the-deal-in-our-plan-a-scenario)
- [https://ai-2040.com/supplements/verification-plan#2026-2028-preparing-deal-optionality](https://ai-2040.com/supplements/verification-plan#2026-2028-preparing-deal-optionality)
- [2026-2028](https://ai-2040.com/supplements/verification-plan#2026-2028-preparing-deal-optionality)
- [https://ai-2040.com/supplements/verification-plan#2029-2030-deal-implementation](https://ai-2040.com/supplements/verification-plan#2029-2030-deal-implementation)
- [2029-2030](https://ai-2040.com/supplements/verification-plan#2029-2030-deal-implementation)
- [https://ai-2040.com/supplements/verification-plan#2031-2036-improving-the-deal](https://ai-2040.com/supplements/verification-plan#2031-2036-improving-the-deal)
- [2031-2036+](https://ai-2040.com/supplements/verification-plan#2031-2036-improving-the-deal)
- [https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification](https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification)
- [Appendix](https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification)
- [The Plan](https://ai-2040.com/supplements/verification-plan)
- [Get Involved](https://ai-2040.com/supplements/verification-plan/get-involved)
- [tiny field](https://nacicankaya.substack.com/p/all-hands-on-deck-to-build-the-datacenter)
- [expression of interest form](https://tally.so/r/LZyyDy)
- [Prepare deal optionality](https://ai-2040.com/supplements/verification-plan#phase-1-prepare-deal-optionality)
- [Implement the deal](https://ai-2040.com/supplements/verification-plan#phase-2-implement-the-deal)
- [Improve robustness](https://ai-2040.com/supplements/verification-plan#phase-3-improve-robustness)
- [1Chiptracking]([object Object])
- [2VerificationR&D funding]([object Object])
- [3Mutual ChipDeclaration]([object Object])
- [4Inference-onlyretrofit]([object Object])
- [5R&Dverification]([object Object])
- [6Hardwarecap & trade]([object Object])
- [7Verificationimprovements]([object Object])
- [Phase 1: Preparing2026–2028]([object Object])
- [Phase 2: Implementing2029–2030]([object Object])
- [Phase 3: Improving2031–2036+]([object Object])
- [Appendix:](https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification)
- [Packet-Based Verification](https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification)
- [https://ai-2040.com/supplements/verification-plan#top-toc](https://ai-2040.com/supplements/verification-plan#top-toc)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects)
- [concentration in datacenter sizes](https://ai-2040.com/supplements/compute-supplement#14-datacenter-sizes)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#section-6-overall-risk-estimate)
- [detection estimates](https://ai-2040.com/supplements/covert-ai-projects#section-3-detecting-covert-ai-infrastructure)
- [transparency supplement](https://ai-2040.com/supplements/transparency-plan)
- [SL5](https://www.rand.org/pubs/research_reports/RRA2849-1.html)
- [Plan A verification story](https://ai-2040.com/supplements/verification-plan#verifying-the-deal-in-our-plan-a-scenario)
- [deal decline supplement](https://ai-2040.com/supplements/deal-decline)
- [deal decline supplement](https://ai-2040.com/supplements/deal-decline#preparing-stockpiles-to-align-incentives)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#section-2-preventing-covert-compute-procurement)
- [offline licensing](https://www.rand.org/content/dam/rand/pubs/working_papers/WRA3000/WRA3056-1/RAND_WRA3056-1.pdf)
- [fixed set](https://www.rand.org/content/dam/rand/pubs/working_papers/WRA3000/WRA3056-1/RAND_WRA3056-1.pdf)
- [hardcoded weights](https://taalas.com/the-path-to-ubiquitous-ai/)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#section-3-detecting-covert-ai-infrastructure)
- [stronger verification concessions](https://ai-2040.com/supplements/covert-ai-projects#section-71-options-for-responding-to-covert-projects)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#section-4-limiting-consortium-uplift-to-covert-projects)
- [covert project risk](https://ai-2040.com/supplements/covert-ai-projects#section-6-overall-risk-estimate)
- [RFIs](https://en.wikipedia.org/wiki/Request_for_information)
- [cryptographic based-approaches](https://pearlresearch.ai/)
- [passive network taps](https://amodo.substack.com/p/tech-note-network-taps-for-ai-verification?triedRedirect=true)
- [reproducible](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
- [here](https://arxiv.org/pdf/2511.20621)
- [appendix](https://ai-2040.com/supplements/verification-plan#appendix-packet-based-verification)
- [bounding the maximum side channel communication](https://techgov.intelligence.org/blog/suppressing-side-channels-in-an-untrusted-data-center-via-retrofitted-defenses)
- [iPhone secure enclaves](https://support.apple.com/guide/security/the-secure-enclave-sec59b0b31ff/web)
- [PUFs](https://en.wikipedia.org/wiki/Types_of_physical_unclonable_function)
- [ZKPs](https://en.wikipedia.org/wiki/Zero-knowledge_proof)
- [covert projects supplement](https://ai-2040.com/supplements/covert-ai-projects#section-2-preventing-covert-compute-procurement)
- [2032 section](https://ai-2040.com/supplements/verification-plan#2032-cap-and-trade-for-robots-and-compute)
- [Zero Data Retention](https://code.claude.com/docs/en/zero-data-retention)
- [Baker et. al.](https://arxiv.org/abs/2507.15916)
- [Scher et. al.](https://arxiv.org/abs/2506.15867)
- [Harack et. al.](https://aigi.ox.ac.uk/publications/verification-for-international-ai-governance/)
- [Cankaya et. al.](https://aigi.ox.ac.uk/wp-content/uploads/2026/04/Fingerprinting_All_AI_Cluster_IO.pdf)
- [Cankaya](https://nacicankaya.substack.com/p/catching-misreporting-about-ml-hardware-bd2)
- [transparency measures](https://ai-2040.com/supplements/transparency-plan)
- [diffuse to potential covert projects](https://ai-2040.com/supplements/covert-ai-projects#section-4-limiting-consortium-uplift-to-covert-projects)
- [economics supplement](https://ai-2040.com/supplements/economics-of-plan-a#core-arguments-for-explosive-growth)
- [current naive modelling](https://ai-2040.com/supplements/economics-of-plan-a#the-full-economic-model)
- [effective](https://ai-2040.com/supplements/compute-supplement#appendix-the-effective-h100e-model)
- [compute supplement](https://ai-2040.com/supplements/compute-supplement#12-all-compute)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#section-31-plan-a-strategies-for-detection)
- [Rinberg et al. (2025)](https://arxiv.org/abs/2511.02620)
- [§A.3](https://ai-2040.com/supplements/verification-plan#a3-the-assurance-curve)
- [§A.2](https://ai-2040.com/supplements/verification-plan#a2-detection-by-random-sampling)
- [2034: Verification improvements](https://ai-2040.com/supplements/verification-plan#2034-verification-improvements)
- [How much slower does takeoff go with 10x less compute?](https://www.lesswrong.com/posts/7jcPg79p3kD5ir3CL/how-much-slower-does-takeoff-go-with-10-less-compute)
- [model](https://ai-2040.com/supplements/covert-ai-projects#how-much-does-takeoff-slow-down-if-you-reduce-compute-by-10x-n)
- [covert project supplement](https://ai-2040.com/supplements/covert-ai-projects#how-we-model-the-ai-capabilities-progress-of-covert-projects)
- [capabilities scaling strategy supplement](https://ai-2040.com/supplements/capability-scaling-strategy)
- [A100 SXM GPU](https://www.nvidia.com/en-us/data-center/a100/)
- [§A.4.1](https://ai-2040.com/supplements/verification-plan#a41-100k-token-backdoor-in-a-100t-token-training-run)
