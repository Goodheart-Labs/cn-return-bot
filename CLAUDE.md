...
- **Run** — one dispatch of the feed workflow: health check, queue triage, enqueue one post if nothing is waiting, process one item, set the alarm for the next run, exit. It never waits and never does a second item.
- **Alarm** — when the next feed run is due, stored by the run in `everything_feed_schedule` (migration 097). pg_cron reads it once a minute and dispatches when it has come. There is no fixed timer any more.
- **Pacing** (`src/everything/pacing.ts`) — the rule that sets the alarm and spreads the feed budget across the UTC day: interval between feed posts = hours left in the day × mean cost of a recent feed post ÷ money left, measured from the last feed post's start (`everything_items.started_at`, migration 095) and recomputed at the end of every run. Reader-requested pages are outside it.
- **Spend cap** — the daily ceiling on LLM cost, $55 by default, $10 of it reserved for reader requests. Pacing spreads the other $45; the cap stays the hard stop.

**Which creators get checked** (GOO-107): exactly two reasons, and nothing is permanent. Either the creator's `everything_projects.priority_until` is still in the future, which a press in the extension or `everything-prioritize` sets to 7 days, or readers read them inside the 14-day window. "Read them" has two definitions and GOO-135 switches between them on a one-way proof: until some creator has ever been visited by two different reader hashes (`everything_two_readers_seen()`), the old rule applies and any visit row ranks a creator; from then on rows without a hash stop counting. The floor is one **regular reader** (a reader hash that opened at least two different pages, `MIN_REGULAR_READERS_TO_WALK_CREATOR`) or, before the proof, two visit rows (`MIN_VISITS_TO_WALK_CREATOR`); below it a creator is not walked however much money is left. The order follows the same switch: regular readers then distinct pages, or visits before the proof. Above the floor there is no fixed cutoff: the walk admits creators from the top of that list until their publishing rates (posts per day, measured from each feed's listing over 14 days) add up to the posts a day the paced budget affords (`admitCreators` in `autoEnqueue.ts`); creators holding priority are always admitted and counted first, the creator who crosses the line is still admitted, and nobody below it is listed. Constants live in `creatorRanking.ts`, numbers come from `everything_creator_attention` (migration 089). A creator in neither set is not ranked. The old permanent list (`everything_followed_feeds`) and the follow-request mailbox (`everything_follow_requests`) are gone, along with the `EVERYTHING_VISIT_CREATORS` switch, which never ran in production. A creator whose feed will not list is skipped for that cycle with one line in the log rather than killing the run; they fall out by themselves when their window or their visits lapse. The extension writes the press straight into `everything_projects` with the anon key, and the database decides the 7 days: anon may insert `feed_url` only, a trigger fills the slug, name and expiry, and a row policy caps the window. The site's sidebar shows only projects that have content, so a press cannot put an empty project on the public page.
...
bun run everything-auto-enqueue [--dry-run] [--affordable <n>]    # enqueue the next unprocessed post from a walked creator; --affordable <n> shows the walk at n posts a day
...
Pipeline: per claim it forces `bot=simple-bot, note_prefilter=off, search_claim=on`; search runs on Sonnet 5 (`simple_bot_search=sonnet5-native`, moved off Opus 5 in August 2026 because it kept exhausting the daily spend cap), the writer on Sonnet 5 (`simple_bot_writer=sonnet5`), the source verifier on Gemini 3 Flash, non-claim-based (`verifier_claim_based=classic`, the single-call accept/reject flow) with `verifier_citations=on` (accepted sources carry a verbatim supporting quote + explanation). Before the checks, two Opus 5 steps run per item (GOO-97): extraction (`extractClaims.ts`) pulls the claims out of the text and says nothing about their truth; rating (`rateClaims.ts`) then gets the full text plus all claims (each as the fact-check will see it: the author's highlighted words plus the surrounding passage, from the shared `claimCheckFields`) in one call at medium reasoning effort with Claude's built-in web search and web fetch tools, and returns one `judgement` per claim on the seven-level scale (certainly true … certainly false). Only claims rated uncertain or worse are fact-checked (`shouldFactCheck`); a claim the rater leaves out counts as uncertain. One call per item is the point: the claims of a post are correlated, and one good source settles many of them, which a per-claim search cannot see. Both steps record their cost in `everything_pipeline_runs` (migrations 088 and 091), so the spend cap counts them. Substack images are fed inline to the Opus claim extractor, so a claim can rest on text, an image, or both (`context_quote` nullable, `image_urls` carries the images).
