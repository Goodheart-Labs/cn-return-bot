...
bun run scrape --incremental # daily mode: from top, catch up oldest known-unscraped note (+1 week buffer), then daily resample
```
...
`--incremental` is the unattended daily mode. It first looks for the oldest known
