...
bun run everything-auto-enqueue [--dry-run]                       # enqueue the next unprocessed priority-feed items (src/everything/priorityFeeds.ts)
...
- **Analytics (PostHog)**: Supabase-first — metrics come from the `everything_*` tables (linked into PostHog as a data-warehouse source, queried via SQL insights) wherever the DB records the fact; events exist ONLY where the DB is blind (anonymous visits/`$pageview`, extension `notes_shown` + `extension_installed`, `sign_in_started`, `vote_gated_login`, the write-note teaser, judge rejections) plus `note_voted` — kept as an event because funnel steps must be events; it carries `distinct_notes_voted` for the ≥5/≥10 funnel steps. Before adding an event, check whether a table already records it. Capture points call `track()` from `everything-shared/analytics.ts` (a sink registry, no-op until a transport registers): the website registers posthog-js (`everything-web/src/lib/analytics.ts` — manual `$pageview` on query-param navigation, anon-id reset only on true sign-out), the extension a fetch transport (`everything-extension/utils/analytics.ts` — posthog-js must NEVER enter its bundles: remote-code store risk, autocapture on host pages; all events route through the background, which owns the ids in `chrome.storage.local` and derives `signed_in`/reset from the `sb-*-auth-token` storage key). Every event is stamped `platform` (web/extension); votes also record it in the DB (migration 069). Key: `VITE_PUBLIC_POSTHOG_KEY` baked in at build (repo secret `POSTHOG_KEY` in both deploy-pages.yml and build-extension.yml; local prod builds read `.env.prod-backend`) — unset means all analytics compiles out.
