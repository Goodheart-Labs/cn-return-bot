...
- **Cycle** — one pass of the feed run: health check, queue triage, the pacing gate, and, when the gate is open, enqueue one post if nothing is waiting and process one item. A dispatch fires every 30 minutes. A run waits in place when the gate opens within 25 minutes and otherwise exits, so a run usually processes 0 or 1 items.
- **Pacing** (`src/everything/pacing.ts`) — the rule that spreads the feed budget across the UTC day: interval between feed posts = hours left in the day × mean cost of a recent feed post ÷ money left, measured from the last feed post's start (`everything_items.started_at`, migration 095) and recomputed before every post. Reader-requested pages are outside it.
