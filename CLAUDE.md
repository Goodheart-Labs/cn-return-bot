...
- **Claim** — a factual statement pulled out of an item. Each is checked separately and each check costs money. A claim may produce a **note**.
- **Visit** — one row the extension writes when a reader opens a post or video, carrying which creator it belonged to, no account and no name, plus a **reader hash** (GOO-135): SHA-256 of a random secret held only on the device combined with the creator's feed address, so it is one value per browser and per creator. That is what lets the pipeline count how many different people read a creator while making the rows unjoinable into one person's reading across creators. Rows written before migration 089, and rows from extension copies that never updated, carry none.
- **Walk** — going through the creators we care about and asking each feed what is new.
- **Cycle** — one pass of consume, enqueue, process. A dispatch fires every 30 minutes and repeats cycles until 5 minutes have elapsed, so a run is usually 1 to 3 cycles.
- **Spend cap** — the daily ceiling on LLM cost, $55 by default.

**Which creators get checked** (GOO-107): exactly two reasons, and nothing is permanent. Either the creator's `everything_projects.priority_until` is still in the future, which a press in the extension or `everything-prioritize` sets to 7 days, or readers read them inside the 14-day window. "Read them" has two definitions and GOO-135 switches between them on a one-way proof: until some creator has ever been visited by two different reader hashes (`everything_two_readers_seen()`), the old rule applies, which is at least two visit rows; from then on a creator needs at least one **regular reader**, meaning one reader hash that opened at least two different pages of theirs, and rows without a hash stop counting. The order follows the same switch: regular readers then distinct pages, or visits before the proof. Constants live in `creatorRanking.ts`, numbers come from `everything_creator_attention` (migration 089). A creator in neither set is not walked. The old permanent list (`everything_followed_feeds`) and the follow-request mailbox (`everything_follow_requests`) are gone, along with the `EVERYTHING_VISIT_CREATORS` switch, which never ran in production. A creator whose feed will not list is skipped for that cycle with one line in the log rather than killing the run; they fall out by themselves when their window or their visits lapse. The extension writes the press straight into `everything_projects` with the anon key, and the database decides the 7 days: anon may insert `feed_url` only, a trigger fills the slug, name and expiry, and a row policy caps the window. The site's sidebar shows only projects that have content, so a press cannot put an empty project on the public page.
...
bun run everything-auto-enqueue [--dry-run]                       # enqueue the next unprocessed post from a walked creator
bun run everything-prioritize <creator-url...>                    # give creators 7 days of priority (same grant as the extension's button)
...
bun run commonnotes-dashboard-local                             # analytics dashboard against the local Supabase
```
...
- **No earnest gate**: user-written notes and improvements post directly, with no LLM check (the `judge-note` edge function was removed on Jim's call, 2026-08-29; undeploy it in prod with `supabase functions delete judge-note`). Moderation is the community's votes and the author's own delete.
...
- **Analytics (Supabase)**: Supabase-first — metrics come from the `everything_*` tables wherever the DB records the fact; events exist ONLY where the DB is blind and land as rows in `everything_events` (migration 077: insert-only for clients, event-name whitelist CHECK, jsonb `props`, `device_id` + nullable `user_id`). Vocabulary: web `pageview`, extension `notes_shown` + `extension_installed` + `extension_active` (a daily heartbeat: the background sends one per install per UTC day, migration 092, so live installs can be counted), and `sign_in_started` / `signed_in` / `vote_gated_login` / the write-note teaser / judge rejections on both. There is no `note_voted` event — votes live in `everything_votes` (with `platform`, migration 069) and the funnel reads them there. Before adding an event, check whether a table already records it. Capture points call `track()` from `everything-shared/analytics.ts` (a sink registry, no-op until a transport registers): the website inserts directly via the shared anon client (`everything-web/src/lib/analytics.ts` — device id in localStorage, initial `pageview` on load + manual dedupe on query-param navigation, device-id reset only on true sign-out), the extension routes every event through the background (`everything-extension/utils/analytics.ts` — background owns `cn-device-id`/`cn-user-id` in `chrome.storage.local`, sources `user_id` from the live session because RLS checks it against the JWT, and derives `signed_in`/reset from the `sb-*-auth-token` storage key). Events on any event row that happens signed-in carry both ids — that pair is the stored device→user link. The dashboard at `/cn-return-bot/analytics/` (`src/analytics-dashboard/`, `bun run commonnotes-dashboard` / `commonnotes-dashboard-local` on port 8004; both first stop whatever listens on 8004 and refuse to fall back to another port) reads live aggregates via security-definer RPCs — the anon key can never read raw events: `everything_metric_series(granularity)` (one row per day/week/month with the seven Common Notes metrics; people metrics come as histograms of "did it exactly k times", capped at 20, GOO-98), `everything_pipeline_daily()` (per day: posts the pipeline finished and, as a cohort, the claims extracted, claims checked and AI-note vote tallies; "rated helpful" is applied client-side with the shared `noteStatus()`), and `everything_funnel(window_days)`. The creators-by-visits leaderboard was removed from the dashboard in September 2026 (the pipeline's run log carries the ranked creator table); `everything_creator_visits` stays in the database unused. `everything_daily_activity` is no longer read. Charts are built from visx primitives (`@visx/*`); the pipeline funnel's time window is `@visx/brush`. No analytics key or secret exists.

Prod prerequisites (manual): run migrations (incl. 058–061; 060's realtime-publication line included); repo secret `SUPABASE_ANON_KEY`. In the prod project's dashboard (Auth → URL Configuration): set **Site URL** to `https://goodheart-labs.github.io/cn-return-bot/notes/` and add it (plus `https://goodheart-labs.github.io/cn-return-bot/**`) to the redirect allow-list — X OAuth redirects there. Mirror the branded email templates (`supabase/templates/confirmation.html`, `magic_link.html`) under Auth → Email Templates; config.toml only styles local dev.
...
bun run build-ext-dev # dev build vs PROD backend, with the self-reload hook and build id compiled in — what Jim's Mac runs
...
**Delivering a change to Jim's Mac Chrome (do this after EVERY extension change, without being asked).** Jim develops on this VPS but browses on his Mac, and he judges a change by seeing it in his real browser. The steps: (1) `bun run build-ext-dev` — never plain `build-ext` for this, because only the dev build compiles in the self-reload poller and the build id; (2) make sure the symlink `~/dev/ext-build` points at YOUR worktree's `src/everything-extension/.output/chrome-mv3-prod-backend` (repoint it with `ln -sfn`, and point it back at a surviving checkout with a fresh build behind it before deleting a worktree it points into); (3) run `~/.local/bin/cn-ext-push`, which copies the build over SSH to the Mac folder Chrome loads unpacked (`/Users/jimmaar/dev/ext-builds/common-notes`). That is the whole delivery: the dev build's background polls its own `build-id.txt` from disk every 30 seconds and reloads itself when a newer build has landed, so nothing opens, navigates, or focuses anything in Jim's browser. Quote the push output and tell Jim the build id (HH:MM:SS); he can confirm it in any freshly loaded page's console as `[common-notes] dev build <id>`. Two rules: an already-open tab keeps its old content scripts (and their CSS) until that tab is refreshed, so tell Jim to refresh before judging a change; and never run `cn-ext-push --verify`, the old probe-tab check, without asking — it opens a tab and yanks his laptop's focus to Chrome, which he explicitly asked to be rid of (2026-08-25).
...
### Replay a bot run from prod logs (`tryoutNotes --from-db [level]`)

Re-run a tweet's most recent run locally, reusing data from its prod
...
from the gates. Defaults to `--bot simple-bot`. See `src/local/seedReplayFromDb.ts`,
`src/pipeline/input/inputCache.ts`, `src/pipeline/replay/writerCache.ts`.
