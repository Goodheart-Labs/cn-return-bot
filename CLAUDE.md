...
Request a Common Note: right-click any selection on ANY page → the background injects `content-scripts/requestnote.js` on demand (the menu click's activeTab grant authorizes it — no host permissions) and a `common-notes-requestnote` shadow-root card confirms the request was logged to `everything_note_requests` (migration 066: insert-only for clients, anon allowed with `user_id` null — a write-only inbox telling us where to expand coverage; the inline-notes-everywhere feature itself isn't built yet). The only static first-class text site is `*://*.substack.com/*` (single source: `utils/staticSites.ts`, mirrored by `notes.content.ts` matches); every other text site (incl. ai-2040.com since Jul 2026) is covered by the background's noted-sites sync. The extension icon is generated from `src/everything-extension/assets/icon.svg` via `bun run scripts/generate-extension-icons.ts` (PNGs checked in under `public/icon/`).
