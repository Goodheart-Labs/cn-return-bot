...
|---|---|---|---|---|---|
| T2 | **Breaking-news gate** (date fix + time-travel test) | **−11–15%** *(backtested 07-28; ex-ante said −20–30%)* | ~−1% (backtested FP) | 398-note counterfactual replay: 9NH vs ~3 real H flagged by the time-travel test; absence-of-reports rule inverted (9H/5NH) and demoted to signal-only | **High (backtested)** |
...
**BACKTEST RUN 2026-07-28 (398 rated notes, Claude judges, only-what-the-bot-saw; `tmp/t2_chunks` + `tmp/t2_judged` + aggregation in session log).** Results vs the ex-ante estimate:
- False positives: **4.7% of H flagged** (15/319) — at target.
- **12.7% of NH flagged** (10/79) — **below the −20–30% estimate**. 4/5 tagged breaking-cluster notes caught (low n).
- Net-of-rated: 60.3% → 63.0% after gate.
- **Spec-changing finding: the absence-of-reports rule is INVERTED** — flags 9H vs 5NH. "No reports she died" notes on breaking rumors usually land *Helpful* (the rumor usually is false); the rule only pays when the bot's search lags reality (rarer). **Do not gate on it.** Keep it as a logged signal / verifier-prompt nudge ("find positive dated evidence before asserting a negative").
- **Surviving gate = the time-travel test alone** (`valid_at_post ∈ {no, unverifiable}`): flags 9NH vs ~3 real H (3 of 6 flagged H are empty-findings artifacts) → **−11–15% NH at ~1% H cost** → ~+1–2 cap/day, not +3–6. Predicted-vs-actual logged: ex-ante overestimated ~2×.
**SHIPPED as [PR #323](https://github.com/Goodheart-Labs/cn-return-bot/pull/323) (2026-07-28, Nathan's ruling): 50/50 A/B `time_travel_prompt`** — one flag, appended sections in both search + writer prompts, off arm byte-identical to prod. Read plan in the PR body: over-abstention guard (days–2wk) → Nathan's tag rate per arm (~4wk) → NH per arm (directional only). Shadow scorer **parked** ("more building than I want to do today") — revisit if the tag-rate check fires. [Correction folded in: the "prompts pass no date" claim was wrong — timestamps in the user message since #186; era split showed the missing piece was the instruction.]
