...
-- to the UTC day boundary where today's spend is what matters.
--
...
-- 101: a backlog post finished 10 hours ago. Extraction 0.50, rating 0.25, two
...
-- 102: a priority-tier post finished 30 hours ago, one check, 1.50.
-- 103: a reader-requested page (priority 2) finished 5 hours ago, 50.00. Not a
--      feed post, so it must not enter the mean, but its spend counts today.
-- 104: a post that errored 2 hours ago after spending 0.40. Not in the mean.
-- 105: a paragraph check finished 3 hours ago, 0.30. Not a whole page.
-- 106: a backlog post finished 5 days ago, 6.00. Inside the 7-day fallback only.
-- 107: queued, never started.
insert into everything_items (id, url, status, priority, checked_scope, processed_at) values
  ('00000000-0000-0000-0000-000000000101', 'https://a.example/101', 'done',   0, 'page',      now() - interval '10 hours'),
  ('00000000-0000-0000-0000-000000000102', 'https://a.example/102', 'done',   1, 'page',      now() - interval '30 hours'),
  ('00000000-0000-0000-0000-000000000103', 'https://a.example/103', 'done',   2, 'page',      now() - interval '5 hours'),
  ('00000000-0000-0000-0000-000000000104', 'https://a.example/104', 'error',  0, 'page',      now() - interval '2 hours'),
  ('00000000-0000-0000-0000-000000000105', 'https://a.example/105', 'done',   0, 'paragraph', now() - interval '3 hours'),
  ('00000000-0000-0000-0000-000000000106', 'https://a.example/106', 'done',   0, 'page',      now() - interval '5 days'),
...
insert into everything_pipeline_runs (item_id, claim_id, kind, cost, created_at) values
  ...
  ('00000000-0000-0000-0000-000000000106', null, 'extraction', 6.00, now() - interval '5 days');
