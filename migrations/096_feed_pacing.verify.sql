...
from everything_feed_pacing(48, 2, 168);
reset role;
...
set role service_role;
select round(mean_post_cost_usd, 4) as mean, sample_posts, sample_hours from everything_feed_pacing(48, 3, 168);
reset role;

\echo '--- 3. with no finished post in either window the mean is null and the count 0'
set role service_role;
select mean_post_cost_usd, sample_posts, sample_hours from everything_feed_pacing(1, 1, 1);
reset role;
...
set role service_role;
select last_feed_started_at is not null as seen, db_now >= last_feed_started_at as clock_ok from everything_feed_pacing(48, 2, 168);
reset role;
...
select * from everything_feed_pacing(48, 2, 168);
\set ON_ERROR_STOP on
reset role;
