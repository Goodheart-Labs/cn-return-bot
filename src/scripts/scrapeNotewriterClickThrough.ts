...
async function scrollToPosition(
  ...
  console.log(
    `   ${prefix} Scrolling to position${targetNoteId ? ` (target <= ${targetNoteId})` : ` (${maxScrolls} scrolls)`}...`,
  );
  ...
  while (totalScrollsDone < maxScrolls) {
    ...
    try {
      batchResult = await currentPage.evaluate(
        async (px: number, count: number) => {
    ...
    if (targetNoteId && batchResult.tweetIds.length > 0) {
      ...
      console.log(
        `   ${prefix} 📍 tweet ${minTweetId} at scroll ${totalScrollsDone} (scrollY=${batchResult.scrollTop}/${batchResult.scrollHeight})`,
      );
      ...
      console.log(
        `   ${prefix} ... ${totalScrollsDone}/${maxScrolls} scrolls (scrollY=${batchResult.scrollTop}/${batchResult.scrollHeight})`,
      );
    }
...
async function scrapeTab(
  ...
  console.log(
    `${prefix} 📐 Page dimensions: ${dims.innerWidth}x${dims.innerHeight} viewport, ${dims.scrollHeight}px scroll height, ${dims.cells} cells visible`,
  );
  ...
  recoveryLoop: while (frameRecoveries <= MAX_FRAME_RECOVERIES) {
    ...
    try {
      while (collectedNotes.size < maxNotes && retryCount <= maxRetries) {
        ...
        for (const cell of cells) {
          ...
          async function waitAndExtractModal(maxWaitMs: number): Promise<ModalData | null> {
            for (let waitMs = 0; waitMs < maxWaitMs; waitMs += 150) {
              const state = await page.evaluate(() => {
                ...
                const hasStatus =
                  /\bCurrently (not )?rated helpful\b/i.test(text) || /\bNeeds more ratings\b/i.test(text);
            ...
            return await page.evaluate(() => {
              ...
              const dateMatch = modalText.match(
                /Note submitted[:\s]*([\d:]+\s*(?:AM|PM)?)\s*[·•]\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},?\s+\d{4})/i,
              );
              const submittedDate = dateMatch ? (dateMatch[2] ?? "") : "";

              const raterTags: string[] = [];
              const tagsMatch = modalText.match(
                /Top tags selected by raters\s*\n([\s\S]*?)(?:\n\s*\n|Note ID|Note submitted|$)/i,
              );
              if (tagsMatch) {
              ...
              return {
                ...
                submittedDate,
                usedFallback,
                raterTags: raterTags.length > 0 ? raterTags : null,
              };
            });
          }
          ...
          // ============================================================
          let tweetData: Awaited<ReturnType<typeof readCellTweetData>> = {
            tweetId: null,
            tweetUrl: null,
            noteIdFromUrl: null,
          };
          let cellData: Awaited<ReturnType<typeof readCellData>> = {
            noteText: "",
            sourceUrl: null,
            viewCount: null,
            shownOnX: null,
          ...
          conflictLoop: for (let conflictRetry = 0; conflictRetry < 3; conflictRetry++) {
            if (conflictRetry > 0) {
            ...
            for (let qualityAttempt = 0; qualityAttempt <= maxQualityRetries; qualityAttempt++) {
              ...
              if (!md?.noteId) {
                ...
                if (qualityAttempt < maxQualityRetries) {
                  console.log(
                    `   ${prefix} 🔄 No note ID in modal — quality retry ${qualityAttempt + 1}/${maxQualityRetries}`,
                  );
                  qualityRetryCount++;
                }
                continue;
              ...
              if (qualityAttempt < maxQualityRetries) {
                ...
                console.log(
                  `   ${prefix} 🔄 Quality retry ${qualityAttempt + 1}/${maxQualityRetries}: missing=[${missingFields.join(", ")}]`,
                );
                qualityRetryCount++;
          ...
          const finalStatus = modalData.status !== "UNKNOWN" ? modalData.status : cellData.cellStatus || "UNKNOWN";

          // Determine tweet_id: real link > post_unavailable > unavailable_noteId
          const finalTweetId =
            tweetData.tweetId || (cellData.postUnavailable ? "post_unavailable" : `unavailable_${modalData.noteId}`);

          const shownInfo =
            cellData.shownOnX === true ? " [Shown on X]" : cellData.shownOnX === false ? " [Not shown on X]" : "";
          const viewInfo = cellData.viewCount ? ` [${cellData.viewCount.toLocaleString()} views]` : "";
          const tweetInfo = tweetData.tweetId
            ? ` tweet:${tweetData.tweetId}`
            : cellData.postUnavailable
              ? " [Post unavailable]"
              : " [No tweet link]";
          const textPreview = cellData.noteText
            ? ` "${cellData.noteText.slice(0, 80)}${cellData.noteText.length > 80 ? "..." : ""}"`
            : "";
          console.log(
            `   ${prefix} ✓ Found Note ID: ${modalData.noteId} (${finalStatus})${shownInfo}${viewInfo}${tweetInfo}${textPreview}`,
          );

          const tweetMetaInfo = cellData.tweetHandle
            ? ` [@${cellData.tweetHandle}${cellData.tweetTime ? ` · ${cellData.tweetTime}` : ""}]`
            : "";
          if (tweetMetaInfo)
            console.log(
              `   ${prefix}   ${tweetMetaInfo}${cellData.tweetText ? ` "${cellData.tweetText.slice(0, 60)}${cellData.tweetText.length > 60 ? "..." : ""}"` : ""}`,
            );
      ...
      if (
        errMsg.toLowerCase().includes("detached frame") ||
...
async function main() {
  ...
  const startFromNoteId = startFromIdx !== -1 && args[startFromIdx + 1] ? BigInt(args[startFromIdx + 1]!) : null;

  // Parse --stop-at <noteId> to stop scraping at a specific note instead of BOTTOM_NOTE_ID
  const stopAtIdx = args.indexOf("--stop-at");
  const stopAtNoteId = stopAtIdx !== -1 && args[stopAtIdx + 1] ? BigInt(args[stopAtIdx + 1]!) : null;
  ...
  if (freshStart) {
    ...
    for (let i = 1; i < existingNotewriterTabs.length; i++) {
      const scrollPos = await existingNotewriterTabs[i]!.evaluate(() => document.documentElement.scrollTop).catch(
        () => -1,
      );
    ...
    activeTargetId = (page.target() as any)?._targetId ?? null;
    console.log(
      `📄 Reusing furthest-scrolled tab (scrollY=${bestScroll}, ${existingNotewriterTabs.length} tab${existingNotewriterTabs.length > 1 ? "s" : ""} found): ${page.url().slice(0, 80)}`,
    );
  } else {
  ...
  autoRestartLoop: while (autoRestartCount <= MAX_AUTO_RESTARTS) {
    ...
    if (currentStartFrom) {
      ...
      try {
        ...
        try {
          ...
          /* will fail at scrapeTab start if still broken */
        }
    ...
    const scrapedNoteIds = [...collectedNotes.keys()].filter((id) => /^\d+$/.test(id)).map((id) => BigInt(id));
    const oldestScraped = scrapedNoteIds.length > 0 ? scrapedNoteIds.reduce((a, b) => (a < b ? a : b)) : null;
  ...
  if (scrapedIds.length >= 2) {
    ...
    if (knownIds.length > 0) {
      ...
      const coverage =
        dbIds.size > 0 ? ((recovered.length / dbIds.size) * 100).toFixed(1) : "N/A (no prior notes in range)";
  ...
  if (scrapedNoteIds.length > 0) {
    ...
    console.log(
      `   bun run src/scripts/scrapeNotewriterClickThrough.ts ${maxNotes} --fresh --start-from ${oldestReached}`,
    );
  }
