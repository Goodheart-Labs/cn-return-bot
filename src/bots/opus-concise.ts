 ...
 * Has stronger prompting around brevity and a smarter retry loop.
 */
...
async function writeNoteConcise(
  {
    text,
    searchResults,
    citations,
  ...
  config: { model: string; currentDate?: string }
) {
  const maxRetries = 3;
  let attempt = 0;
...
export const opusConcise: Bot = {
  ...
  async runPipeline(post, content): Promise<PipelineResult | null> {
    ...
    try {
      ...
      const noteResult = await writeNoteConcise(
        {
