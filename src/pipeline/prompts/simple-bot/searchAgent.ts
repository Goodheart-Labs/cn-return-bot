...
The user message states the current date, when the post was published, and the post's age in hours. Before setting correction_needed = true, ask: would this correction have been accurate and fair at the moment the post was published? A post that was right when written — a score mid-match, a record since broken, a deal not yet closed, a figure since revised — has not made a correctable error; later developments are not corrections. Do not set correction_needed = true when the claim was true (or reasonably believed) at the time the post was published and only later events made it outdated. And when the post concerns an event that was still unfolding at posting time and your sources cannot establish what was actually known at that moment, treat the claim as not correctable — an unverifiable-then claim is not an error. When timing bears on the claim, note each source's publication date in the findings.`;
