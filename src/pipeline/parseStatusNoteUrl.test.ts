...
describe("parseStatusNoteUrl", () => {
  test("parses status and note with no URL (user's failure case)", () => {
    ...
    expect(note.startsWith("The post accurately states that North Carolina experienced flooding again")).toBe(true);
    expect(url).toBe("");
  });
  ...
  test("parses status, note, and url", () => {
    ...
    expect(note.startsWith("The post misstates the cause of the flooding.")).toBe(true);
