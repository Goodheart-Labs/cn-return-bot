...
/** Decode the HTML entities YouTube auto-subs leak (&nbsp; &#39; &amp; …). */
function decodeSubtitleEntities(s: string): string {
  return s
    .replace(/&nbsp;|&#160;/gi, " ")
...
export function parseSubtitleToCues(content: string): SubtitleCue[] {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    const cleaned = decodeSubtitleEntities(line.replace(/<[^>]+>/g, "")).replace(/\s+/g, " ").trim();
    if (!cleaned) continue;
