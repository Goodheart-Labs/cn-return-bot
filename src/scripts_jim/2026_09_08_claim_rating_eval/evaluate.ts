...
async function evaluateItem(itemId: string) {
  ...
  console.log(`\nRated ($${rating.cost.cost.toFixed(2)}, ${rating.webSearches} web searches, ${rating.cost.input_tokens} in / ${rating.cost.output_tokens} out tokens)\n`);
  ...
  writeFileSync(
    ...
    JSON.stringify({ itemId, title: item.title, raterCost: rating.cost, webSearches: rating.webSearches, comparisons }, null, 2),
  );
}
