...
await Promise.all(data!.map(async (t) => {
  ...
  await withTweetLog(log, () => withBotConfig(PREFILTER_CONFIG, () => withCostTracker(async () => {
    ...
    try { const s = await runSatireDetector(t.text); console.log(`${stamp()} [${t.tweet_id}] satire: ${JSON.stringify(s).slice(0, 120)}`); } catch (e: any) { console.log(`${stamp()} [${t.tweet_id}] satire threw: ${e.message}`); }
