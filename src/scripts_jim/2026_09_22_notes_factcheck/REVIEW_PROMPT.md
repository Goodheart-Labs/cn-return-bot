...
1. Read the whole post, not only the passages next to the notes. If `full_text` is null, fetch the source yourself: Dwarkesh episodes have a transcript at `https://www.dwarkesh.com/p/<guest-slug>` (search for it), YouTube videos can be read through a transcript site. Load the fetch tool first with ToolSearch `select:WebFetch`. For searching, run `bun run src/scripts_jim/2026_09_22_notes_factcheck/search.ts "<query>"` from the repository root; it prints Google results through the pipeline's Serper client. Do not use the built-in WebSearch tool, its session budget is used up.
