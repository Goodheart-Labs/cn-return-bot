...
Jim's guess is right: this is the standard, and it has been since WebGPT in 2021. Three patterns exist in production harnesses, and the good ones combine two of them.

**Pattern 1: viewport plus find.** The harness keeps the whole page on its side and shows the model one numbered window at a time; a `find` call returns the lines around each match. The vocabulary comes from OpenAI: WebGPT had `Find in page`, `Scrolled down`, `Quote` (https://ar5iv.labs.arxiv.org/html/2112.09332); ChatGPT browsing had `open_url`, `scroll`, `quote_lines` (https://github.com/spdustin/ChatGPT-AutoExpert/blob/main/_system-prompts/browse-with-bing.md); and the open-source `gpt-oss` browser tool, which is the interface those models were trained with, has exactly three functions, `search`, `open(id, loc, num_lines)` and `find(pattern)`, with a view capped at 1,024 tokens, lines prefixed `L12:`, and citations of the form `【6†L9-L11】` (https://developers.openai.com/cookbook/articles/openai-harmony, https://github.com/openai/gpt-oss/tree/main/gpt_oss/tools/simple_browser). The gpt-oss README states the reason plainly: "To control the context window size this tool uses a scrollable window of text that the model can interact with." The HuggingFace Open Deep Research browser (https://github.com/huggingface/smolagents/blob/main/examples/open_deep_research/scripts/text_web_browser.py) is the same idea with 8,192-character pages and the tools `visit_page`, `page_up`, `page_down`, `find_on_page_ctrl_f`, `find_next`. Every coding agent reads files the same way: Claude Code's `Read(offset, limit)` plus `Grep`, Codex, Gemini CLI, and SWE-agent's 100-line viewer with `search_file`.

**Pattern 2: page in a file, model uses grep.** The page never enters the context; the model asks for slices. LangChain Deep Agents write any tool result over 20,000 tokens to a file and replace it in context with a pointer and a preview (https://docs.langchain.com/oss/python/deepagents/context-engineering); Vercel's bash-tool and Manus do the same; Anthropic's "dynamic filtering" and programmatic tool calling let Claude write code that filters a fetched page before it reaches the context (https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling).

**Pattern 3: return only relevant passages.** A second, cheaper step reads the page for a goal and returns excerpts: Claude Code's own `WebFetch` (a small model answers a prompt about the page), Gemini CLI's `web_fetch`, Tongyi DeepResearch's `visit(url, goal)`, and on the API side Exa `highlights`, Parallel `excerpts`, Tavily `chunks_per_source` and Perplexity's snippets. The reference MCP `fetch` server is the weakest form: offset pagination with `start_index` and `max_length` and no find, so a 40,000-character article is eight blind pages (https://github.com/modelcontextprotocol/servers/blob/main/src/fetch/README.md).

**Evidence.** Nobody has published a controlled comparison of window-plus-find against full page against highlights on the same web task with the same model, and nobody publishes results for models at Muse Spark's price. The closest results all point the same way:

| Finding | Numbers | Who ran it |
| --- | --- | --- |
| A bounded window with search beats both a tiny window and the full file | SWE-bench Lite: 100-line window 18.0%, 30-line 14.3%, full file 12.7%; a search that refuses to list more than 50 hits 18.0% vs listing them all 12.0% (https://arxiv.org/html/2405.15793v3) | Academic (SWE-agent) |
| Snippets alone miss answers; a full-document tool used sparingly helps | Giving the agent a `get-document` tool raised accuracy from 35.4% to 43.6% with 1.85 full reads per query; 13.5% of gold answers sit past the first 512 tokens (https://arxiv.org/html/2508.06600) | Academic (BrowseComp-Plus) |
| Irrelevant text in the context lowers accuracy for every model, including cheap ones | 18 models; even one distractor lowers accuracy, four compound it (https://www.trychroma.com/research/context-rot) | Vendor (Chroma) |
| Filtering fetched pages in code raises accuracy and cuts tokens | BrowseComp: Sonnet 4.6 33% to 47%; "11% accuracy improvement while using 24% fewer input tokens" (https://claude.com/blog/improved-web-search-with-dynamic-filtering) | Vendor (Anthropic) |
| Relevant passages match full text at a fraction of the tokens | 500 characters of highlights match the first 8,000 characters of the page on SimpleQA (https://exa.ai/blog/highlights-for-agents) | Vendor (Exa) |
| Indirection hurts: results the model must fetch from a file score lower than inline results, badly for some pairs | LongMemEval: Codex + GPT-5.4 from 93% inline to 55% file-based; Claude Code + Opus 4.6 from 77% to 68% (https://arxiv.org/html/2605.15184v1) | Consultancy (PwC) |

The last row is the warning for us. Pattern 2 (files and grep) costs accuracy when the model has to take an extra step to reach content, and it is worst with weaker model-and-harness pairs, which is our case. Pattern 1 keeps the first view inline, so the extra step only happens when the first view misses.

**Where we stand.** Our `web_fetch` returns up to 20,000 characters from the top of the page with no line numbers, no window and no find. A quote after character 20,000 cannot be found at all, and 10% to 30% of fetched pages are cut there (section 2.1). The verifier receives the same 20,000-character dump per cited source and has no way to check that the "supporting quote" it returns is verbatim.

**Recommended shape** (from the research, adapted to our loop). Fetch once per URL per run into a cache of numbered lines, re-wrapped so no line exceeds about 300 characters, with a normalised copy of each line (the same `normalizeText` the timestamp snapping and the extension anchoring use) for matching. Three tools replace the current one:

- `web_fetch(url, query?)` shows the first view, about 6,000 characters (roughly 1,500 tokens, between gpt-oss's 1,024 tokens and smolagents' 8,192 characters). With `query`, the view starts at the passage with the best lexical overlap, so in the common case one call is enough and the model does not have to learn to search. The header says how many lines the page has and how to see more.
- `web_open(url, start_line, num_lines)` shows another window of an already fetched page. Free, no network.
- `web_find(url, pattern)` returns each case-insensitive, punctuation-insensitive match with two lines of context, capped at 20 matches with "use a more specific phrase" above that (SWE-agent's rule). No regex; cheap models write bad regex.

Two additions the research argues for on top: the verifier answers with `{ url, quote, line_start, line_end }` and the harness checks the normalised quote is a substring of those lines before accepting it, which turns WebGPT's `Quote` step into a check no model call is needed for; and once a newer view of the same URL is in the context, older views of it collapse to one line, which is SWE-agent's "keep the last observations" rule.

**What to expect.** Page tokens per fetch drop from up to 5,000 to about 1,500 per view, and because every turn re-sends the history, the summed input tokens over a loop drop by more than half. At Muse's prices that is fractions of a cent per claim, so the money is small. The wins that matter are that quotes beyond the cap become reachable, that the verifier can reject paraphrased quotes outright, and that the runaway pattern of refetching the same URL becomes free. The risk is that a cheap model never calls `web_find` and answers from the first window; the `query` parameter and the quote check are the two mitigations. It should ship as an A/B arm against the current tool and be judged on verifier acceptance, quote-anchoring rate, tokens per check and turns per check.
