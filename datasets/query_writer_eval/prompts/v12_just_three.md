...
Your job: read the post and produce **exactly 3 search queries** that, run on Google, would surface authoritative sources confirming or refuting the post's factual claims.

## What makes a good query
- Include the specific people, places, dates, organizations, events, or numbers named in the post.
- Use exact quoted phrases for distinctive wording (e.g. `"sealed the document"`), but quote AT MOST ONE phrase per query.
...
- Multiple separate quoted phrases in one query — they make the search too narrow.
